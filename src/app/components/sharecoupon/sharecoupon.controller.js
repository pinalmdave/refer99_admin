  ...
  function SharecouponController($scope, User, Couponlist, $location, $uibModal, $log, $document, $stateParams, Socialshare, business_logo) {
      ...
      User.getCampaign(cp_id, function(err, res) {
        if (err) {
          ...
          if (sharecoupon.camp_data && sharecoupon.camp_data.members && sharecoupon.camp_data.members.business_logo) {
            ...
            sharecoupon.business_logo = business_logo + "fb_share.png";
          }
    ...
    sharecoupon.onFacebook = function() {
      ...
      FB.ui({
          ...
          picture: sharecoupon.business_logo,
          caption: 'Reference Documentation',
          description: sharecoupon.description,
          message: 'Facebook Dialogs are easy!'
        },
        function(response) {
          if (response && response.post_id) {
            // alert('Post was published.');
            sharecoupon.socialShare = true;
          } else {
            sharecoupon.socialShare = false;
    ...
    function generateProcessCoupon(email, contact) {
      toastr.info('Just generating coupon', 'Please Wait..', {
        timeOut: 0,
        extendedTimeOut: 0
      });
      var data = {
      ...
      Couponlist.process_coupon_generate(data, function(err, res) {
        toastr.clear();
        if (err) {
          if (err.data && err.data.error && err.data.error.message) {
            sharecoupon.errorMessage = err.data.error.message;
          ...
          sharecoupon.errorMessage = false;
          sharecoupon.coupon_data = res.result;
          sharecoupon.shareurl = "http://refer99.com/admin/#/app/" + cp_id + "/" + sharecoupon.coupon_data.refer_id + "/coupon_share";
          // sharecoupon.shareurl = "http://refer99.com";
          if (sharecoupon.camp_data.cp_share || sharecoupon.camp_data.add_discount) {
            sharecoupon.openSocialShareModal();
          }
        }
      });
    }
    ...
    sharecoupon.openGetCouponModal = function() {
    ...
    sharecoupon.openSocialShareModal = function() {
      var modalInstance = $uibModal.open({
        animation: true,
        ariaLabelledBy: 'modal-title',
        ariaDescribedBy: 'modal-body',
        ...
        // appendTo: parentElem,
        controller: function($scope) {
          $scope.m_sharecoupon = sharecoupon;
          $scope.close = function() {
            modalInstance.close();
          }
          $scope.shareOnFacebook = function() {
            sharecoupon.onFacebook();
            modalInstance.close();
          }
        }
      });
